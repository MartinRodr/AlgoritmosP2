...
void medir_tiempos_ins(int n) {
	int *v = malloc(n * sizeof(int));
	double t_ini, t_fin;

	// Caso (a): Vector ordenado ascendente
	for (int i = 0; i < n; i++) v[i] = i;
	t_ini = microsegundos();
	ord_ins(v, n);
	t_fin = microsegundos();
	printf("Ascendente: n=%d, tiempo=%.3f us\n", n, t_fin - t_ini);

	// Caso (b): Vector ordenado descendente
	for (int i = 0; i < n; i++) v[i] = n - i;
	t_ini = microsegundos();
	ord_ins(v, n);
	t_fin = microsegundos();
	printf("Descendente: n=%d, tiempo=%.3f us\n", n, t_fin - t_ini);
	...
	ord_ins(v, n);
	t_fin = microsegundos();
	printf("Desordenado: n=%d, tiempo=%.3f us\n", n, t_fin - t_ini);

	free(v);
}

// Tabla para inicialización ascendente
void tabla_tiempos_ascendente() {
...
// Tabla para inicialización descendente
void tabla_tiempos_descendente() {
    int tamanos[] = {500, 1000, 2000, 4000, 8000, 16000, 32000};
    int num_tamanos = sizeof(tamanos) / sizeof(tamanos[0]);
...
// Tabla para inicialización aleatoria
void tabla_tiempos_aleatorio() {
    int tamanos[] = {500, 1000, 2000, 4000, 8000, 16000, 32000};
    int num_tamanos = sizeof(tamanos) / sizeof(tamanos[0]);
...
int main(void){
	...
	p2();
	p3();
	tabla_tiempos_ascendente();
	tabla_tiempos_descendente();
	tabla_tiempos_aleatorio();
	return 0;
}